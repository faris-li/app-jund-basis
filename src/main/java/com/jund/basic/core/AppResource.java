...
public class AppResource extends BaseController {
	...
	@Logging(title = "删除应用",data = "删除应用{0}")
	...
	@Logging(title = "新增或修改应用",data = "新增或修改应用{0}")
	...
	public ResponseInfo save(@RequestBody App app) {
		if(app == null ){
			throw new VpRuntimeException(RestConst.ReturnCode.ERROR, "应用为空!");
		}
		appService.save(app);
		return new ResponseInfo();
	}
