...
import com.jund.basis.core.entity.Organ;
import com.jund.basis.core.entity.User;
import com.jund.basis.core.entity.dto.OrganDTO;
import com.jund.basis.core.repository.OrganRepository;
import com.jund.basis.core.repository.UserRepository;
import com.jund.basis.core.service.OrganService;
...
public class OrganServiceImpl extends BaseServiceImpl<Organ, Long> implements OrganService {
    ...
    public List<Organ> findOrganByPidAndStatus(Long pid, Integer... status) {
        ...
        if (!ArrayUtils.isEmpty(status)) {
            sql += " and status = " + Const.DICT.INTEGER_TRUE;
        }
    ...
    public void isCancelStatus(Long[] ids) {
        ...
        for (Long id : ids) {
             ...
             * 获取所有后代机构
             */
            List<Organ> childList = findAllChildByPseq(organ.getOrgSeq(), Const.DICT.INTEGER_TRUE);
    ...
    @Transactional
    @Logger(title = "启用&撤销", data = "将机构{0}状态改为{1}")
    public void updateStatus(Long[] ids, Integer status) {
    	if(status == Const.DICT.INTEGER_TRUE){
    		isStartStatus(ids);
    	}else{
    		isCancelStatus(ids);
    	}
        ...
        for (Long id : ids) {
            if (status == Const.DICT.INTEGER_TRUE) {
                ...
                Organ organ = findOne(id);
                List<Organ> allChildren = findAllChildByPseq(organ.getOrgSeq(),Const.DICT.INTEGER_TRUE);
                for (Organ child : allChildren) {
                    child.setStatus(Const.DICT.INTEGER_TRUE);
    ...
    @Transactional
    @Logger(title = "保存机构信息", data = "机构信息{0},上级机构{1}")
    ...
    private List<OrganDTO> converToDto(List<Organ> orgList) {
        ...
        for (Organ organ : orgList) {
            ...
            try {
                BeanUtils.copyProperties(dto, organ);
            } catch (Exception e) {
                LOGGER.error("", e);
            }
            if (null != organ.getParentOrgan()) {
